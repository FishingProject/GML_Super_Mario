...
// obj_brick
// jump
if (place_meeting(x,y+1,object_brick))
{
	vsp = key_jump * -jumpspeed;
}

// vertical collision
if (place_meeting(x,y+vsp,object_brick))
{
	while(!place_meeting(x,y+sign(vsp),object_brick))
	{
		y += sign(vsp);
	}
	vsp = 0;
...
if (place_meeting(x+hsp,y,object_brick))
{
	while(!place_meeting(x+sign(hsp),y,object_brick))
	{
		x += sign(hsp);
	}
	hsp = 0;
